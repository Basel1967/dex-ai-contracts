...
def main():
 ...
 status='PASS' if not F else 'FAIL'; report={'session':'SESSION 22','contract':'Raw Event 1.0.0','status':status,'passed':len(P),'failed':len(F),'results':P+F}; (R/'reports/session22-raw-event-conformance-report.json').write_text(json.dumps(report,indent=2)+'\n'); print(json.dumps({'status':status,'passed':len(P),'failed':len(F)},indent=2)); return 0 if not F else 1
